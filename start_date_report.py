...
def get_same_or_newer(start_date,startlist):
...
def list_newer(start_date):
    ...
    data = get_file_lines(FILE_URL)
    reader = csv.reader(data[1:])
    startlist = list(reader)
    startlist.sort(key=itemgetter(3))
    print(startlist)
    while start_date < datetime.datetime.today():
        start_date, employees = get_same_or_newer(start_date,startlist)
